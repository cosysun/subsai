...
def download_youtube_video(url):
    try:
        ...
        #     print(f"{i}. Resolution: {stream.resolution}, Size: {
        #           size: .2f} MB, Type: {stream_type}")

        # choice = int(
        #     input("Enter the number of the video stream to download: "))
        ...
        video_file = selected_stream.download(
            output_path='./assets/video', filename_prefix="video_")

        if not selected_stream.is_progressive:
            print("Downloading audio...")
            audio_file = audio_stream.download(
                output_path='./assets/video', filename_prefix="audio_")

            print("Merging video and audio...")
            output_file = os.path.join(
                './assets/video', f"{yt.title}.mp4")
        ...
        print(f"Downloaded: {yt.title} to './assets/video' folder")
